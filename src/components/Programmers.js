...
export default function Programmers() {
  ...
  let [featProgrammer, setFeatProgrammer] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    // of the program, without needing to inject the information through arguments.
      return programmers.map((dev) => {
        setFeatProgrammer(dev.name)
      });
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFeatProgrammer(dev.name) }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          featProgrammer
            ? `🎉 Let's celebrate ${featProgrammer}! 🥳`
