...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmersList[proId-1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: proId ? 'gold' : 'royalblue' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ setProId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
